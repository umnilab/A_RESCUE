...
public class ThreadedScheduler_old {
	...
	public synchronized void paraStep() {
		...
		while (!this.roadFinishedStepping){
			try{
				...
				ContextCreator.stopSim(e, ThreadedScheduler_old.class);
			}
		}
	}
...
class ThreadController implements Runnable {
	private ThreadedScheduler_old cc;
	private int numCPUs;
	private boolean[] cpuStatus;
	
	public ThreadController(ThreadedScheduler_old cc){
		this.cc = cc;
					...
					synchronized(this){
						cpus: for (int i = 0; i< this.numCPUs; i++){
							if (this.cpuStatus[i]){
								...
								System.out.println("CPU: " + i + " Allocated to Road: " + r.getLinkid());
